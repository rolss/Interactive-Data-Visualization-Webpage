...
# run_query gives a list of tuples, where each tuple represents a row of the table returned by the query
...
# AVG is chosen as default measure and can be later changed by the user
measurepicked="AVG"
longqueryh=f"""
...
# Variables in raw form are readable by the dataframe and SQL language
# Variables in clean form have the objective of being read by the user
...
row13_1, row13_spacer2, row13_2 = st.columns((5, 3, 3.4)) # Will be constantly used to divide the page into columns.
with row13_2:
...
with row13_1:
    # Turn the stat selected from clean to raw, so it's readable by the dataframe
    column_selected = stats_clean.index(statpicked)
    statpicked = stats_raw[column_selected]

    # Turn the function selected from clean to raw, so it's readable by SQL language
    ...
    # Query that returns a value for every team, based on an SQL aggregate function. These teams play Away
    longquerya=f"""
    ...
    # Query that returns a value for every team, based on an SQL aggregate function. These teams play At Home
    longqueryh=f"""
    WITH hometeam(team, Goals, Half_time_goals, Shots_on_goal, Distance, Total_passes, Succesful_Passes, Failed_passes, Pass_ratio, Possession, Tackle_ratio, Fouls, Fouls_received, Offside, Corners) as (
	SELECT h_team, {measurepicked}(h_goals), {measurepicked}(h_ht_goals), {measurepicked}(h_shots_on_goal), {measurepicked}(h_distance), {measurepicked}(h_total_passes),{measurepicked}(h_success_passes), 
		{measurepicked}(h_failed_passes), {measurepicked}(h_pass_ratio), {measurepicked}(h_possession), {measurepicked}(h_tackle_ratio), {measurepicked}(h_fouls), 
    ...
    # Dataframes for the two previous queries 
    dfa=pd.read_sql_query(longquerya,conn)
    dfh = pd.read_sql_query(longqueryh,conn)

    # Conditional controls type of bar chart displayed, as well as the matches showed based on game type picked
...
row2_1, rowspacer2_1, row2_2 = st.columns((5, 3, 3.4))
with row2_2:
    # Options selector for the user to choose a team and type of game
    st.subheader("Options Selector")
...
# Queries based on the selected team by the user, and relevant to a team playing at home or away.
...
# Deletes certain information that can't be used in the Radar Chart because of the difference in digit range
...
with row2_1:
    # Conditional that controls the matches showed based on the type of game picked
...
row3_1, rowspacer3_1, row3_2 = st.columns((5, 3, 3.4))
with row3_2:
    # Options selector to display all goalkeepers' information
    st.subheader("Options Selector")
    gkvalues_raw = ['NotConceded', 'Conceded', 'PKsFaced', 'Saves']
    gkvalues_clean = ['Not Conceded', 'Conceded', 'Penalty Kicks Faced', 'Saves (percentage)']
...
with row3_1:
    # Queries depending on the order selected by the user and the value selected by the user
    ...
    # Conditional controls the display of a horizontal or vertical chart for better fit, depending on the value selected
    if auxindex <= 2:
        fig = px.bar(dfgk, x=selectedvalue, y='Goalkeeper', width=800, height=1000, template='plotly_dark',color_discrete_sequence=px.colors.qualitative.Dark2)
        
        st.plotly_chart(fig)
    else:
        fig = px.bar(dfgk, x='Goalkeeper', y=selectedvalue, width=800, height=1000, template='plotly_dark',color_discrete_sequence=px.colors.qualitative.Dark2)
        st.plotly_chart(fig)

st.title("Individual Penalty Statistics for Goalkeepers")
row4_1, rowspacer4_1, row4_2 = st.columns((5, 2.3, 3.4))
with row4_2:
    # Option selector for Goalkeepers' Radar Chart
    st.subheader("Options Selector")
    playernames_query="""
    SELECT Goalkeeper
    FROM goalkeepers_penalties
...
with row4_1:
    # Editable query based on Goalkeeper selected previously by the user
    ...
    # Radar chart creation based on Goalkeeper selected, using the previous query
...
with row5_1:
    # ([![Foo](Link pertinente a la imagen)](link al que redirecciona al oprimir la imagen))
